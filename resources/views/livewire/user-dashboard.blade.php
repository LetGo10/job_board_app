<div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
        <div class="bg-gradient-to-r from-blue-50 to-blue-100 p-6 rounded-2xl shadow hover:shadow-lg transition text-center">
            ...
            <p class="mt-3 text-3xl font-extrabold text-gray-800">{{ auth()->user()->jobApplications()->count() ?? 0 }}</p>
        ...
        <div class="bg-gradient-to-r from-green-50 to-green-100 p-6 rounded-2xl shadow hover:shadow-lg transition text-center">
            ...
            <p class="mt-3 text-3xl font-extrabold text-gray-800">{{ \App\Models\Job::count() ?? 0 }}</p>
        ...
        <div class="bg-gradient-to-r from-purple-50 to-purple-100 p-6 rounded-2xl shadow hover:shadow-lg transition text-center">
            ...
            <p class="mt-3 text-3xl font-extrabold text-gray-800">{{ auth()->user()->jobApplications()->count() > 0 ? 'Active' : 'New' }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white p-6 rounded-2xl shadow hover:shadow-lg transition lg:col-span-2">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('user.applications') }}" class="text-sm text-purple-600 hover:text-purple-800">
                    View All →
                </a>
            </div>

            @if(auth()->user()->jobApplications()->count() > 0)
                <div class="space-y-3">
                    @foreach(auth()->user()->jobApplications()->with('job')->latest()->limit(3)->get() as $application)
                        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                            <div>
                                <h4 class="font-medium text-gray-800">{{ $application->job->title }}</h4>
