        ...
        <div class="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div class="relative -mt-16 px-6 lg:px-10">
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 flex flex-col lg:flex-row items-center lg:items-start gap-6">
                    <!-- Avatar -->
                    <img src="https://ui-avatars.com/api/?name={{ urlencode($jobsapply?->full_name) }}&size=150&background=random"
                    ...
                    <div class="flex-1 text-center lg:text-left">
                        ...
                        <p class="text-gray-600 dark:text-gray-400 mt-1">
                            {{ $jobsapply?->job?->title }} @ {{ $jobsapply?->job?->company }}
                        </p>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
                        ...
                        <div class="grid grid-cols-1 gap-y-3 text-gray-700 dark:text-gray-300 text-sm">
                            <div><span class="font-medium">Email:</span> {{ $jobsapply?->email }}</div>
                            <div><span class="font-medium">Phone:</span> {{ $jobsapply?->phone_number }}</div>
                            <div><span class="font-medium">Location:</span> {{ $jobsapply?->job?->location }}</div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
                        ...
                        <ul class="space-y-2 text-gray-700 dark:text-gray-300 text-sm">
                            <li>
                                ...
                                RM {{ !empty($jobsapply?->expected_salary) ? $jobsapply->expected_salary : '0.00' }}
                            </li>
                            <li><span class="font-medium">Available Start:</span> {{ $jobsapply?->available_start_date->format('M d, Y') }}</li>
                            <li><span class="font-medium">Willing to Relocate:</span> {{ $jobsapply?->willing_to_relocate ? 'Yes' : 'No' }}</li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
                        ...
                        <ul class="space-y-2 text-sm">
                            ...
                            <li>
                                <div><span class="font-medium">Cover Letter:</span> {{ $jobsapply?->cover_letter }}</div>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
                        ...
                        <p class="text-gray-700 dark:text-gray-300 whitespace-pre-line text-sm">
                            {{ $jobsapply?->why_interested ?? 'No motivation provided.' }}
                        </p>
                    </div>
                </div>
