        ...
        <div x-transition:enter="transition-all duration-600 ease-out delay-100"
            ...
            <div class="bg-gradient-to-r from-purple-800 to-purple-900 text-white p-8 rounded-t-lg">
                <div class="flex items-start space-x-6">
                    ...
                    <div class="flex-shrink-0">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode($jobsapply?->full_name) }}&size=120&background=8b5cf6&color=ffffff&bold=true"
                             alt="{{ $jobsapply?->full_name }}"
                             class="w-24 h-24 rounded-full border-4 border-purple-300/30 shadow-lg object-cover">
                    </div>

                    <!-- Header Information -->
                    <div class="flex-1">
                        <h1 class="text-3xl font-bold mb-2">{{ $jobsapply?->full_name }}</h1>
                        <p class="text-xl text-purple-200 mb-3">Candidate for {{ $jobsapply?->job?->title }}</p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                            <div class="flex items-center space-x-2">
                                ...
                                <span>{{ $jobsapply?->email }}</span>
                            </div>
                            <div class="flex items-center space-x-2">
                                <svg class="w-4 h-4 text-purple-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path>
                                </svg>
                                <span>{{ $jobsapply?->phone_number }}</span>
                            </div>
                            @if($jobsapply?->linkedin_url)
                            <div class="flex items-center space-x-2">
                                ...
                                <a href="{{ $jobsapply->linkedin_url }}" target="_blank" class="hover:text-purple-200 underline">LinkedIn Profile</a>
                            </div>
                            @endif
                            @if($jobsapply?->portfolio_url)
                            <div class="flex items-center space-x-2">
                                <svg class="w-4 h-4 text-purple-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03-3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9"></path>
                                </svg>
                                <a href="{{ $jobsapply->portfolio_url }}" target="_blank" class="hover:text-purple-200 underline">Portfolio Website</a>
                            </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="text-right">
                        <span class="inline-block px-3 py-1 bg-purple-600 text-purple-100 text-sm font-medium rounded-full">
                            Applied {{ $jobsapply?->created_at?->diffForHumans() }}
                        </span>
                    </div>
                </div>
            </div>
                ...
                <div class="w-80 bg-purple-50 dark:bg-purple-900/20 p-6 space-y-6 border-r border-purple-200 dark:border-purple-700">
                    ...
                    <div>
                        ...
                        <div class="space-y-3 text-sm">
                            <div class="bg-white dark:bg-purple-800/30 p-3 rounded-lg">
                                ...
                                <p class="font-semibold text-purple-900 dark:text-purple-100">{{ $jobsapply?->job?->title }}</p>
                            </div>
                            <div class="bg-white dark:bg-purple-800/30 p-3 rounded-lg">
                                <span class="text-purple-600 dark:text-purple-400 font-medium">Company:</span>
                                <p class="font-semibold text-purple-900 dark:text-purple-100">{{ $jobsapply?->job?->company }}</p>
                            </div>
                            <div class="bg-white dark:bg-purple-800/30 p-3 rounded-lg">
                                <span class="text-purple-600 dark:text-purple-400 font-medium">Location:</span>
                                <p class="font-semibold text-purple-900 dark:text-purple-100">{{ $jobsapply?->job?->location }}</p>
                            </div>
                            <div class="bg-white dark:bg-purple-800/30 p-3 rounded-lg">
                                <span class="text-purple-600 dark:text-purple-400 font-medium">Applied Date:</span>
                                <p class="font-semibold text-purple-900 dark:text-purple-100">{{ $jobsapply?->created_at?->format('M d, Y \a\t H:i') }}</p>
                            </div>
                            @if($jobsapply?->updated_at && $jobsapply->updated_at != $jobsapply->created_at)
                            <div class="bg-white dark:bg-purple-800/30 p-3 rounded-lg">
                                <span class="text-purple-600 dark:text-purple-400 font-medium">Last Updated:</span>
                                <p class="font-semibold text-purple-900 dark:text-purple-100">{{ $jobsapply?->updated_at?->format('M d, Y \a\t H:i') }}</p>
                            </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-3 text-sm">
                            <div class="bg-white dark:bg-purple-800/30 p-3 rounded-lg">
                                ...
                                <p class="font-bold text-green-600 dark:text-green-400 text-lg">💰 RM {{ $jobsapply?->expected_salary ?: 'Negotiable' }}</p>
                            </div>
                            <div class="bg-white dark:bg-purple-800/30 p-3 rounded-lg">
                                <span class="text-purple-600 dark:text-purple-400 font-medium">Available Start:</span>
                                <p class="font-semibold text-purple-900 dark:text-purple-100">📅 {{ $jobsapply?->available_start_date?->format('M d, Y') }}</p>
                            </div>
                            <div class="bg-white dark:bg-purple-800/30 p-3 rounded-lg">
                                <span class="text-purple-600 dark:text-purple-400 font-medium">Willing to Relocate:</span>
                                <p class="font-semibold {{ $jobsapply?->willing_to_relocate ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                                    {{ $jobsapply?->willing_to_relocate ? '✅ Yes' : '❌ No' }}
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="space-y-3">
                            <div class="bg-white dark:bg-purple-800/30 p-3 rounded-lg">
                                <div class="flex items-center justify-between">
                                    ...
                                    @if($jobsapply?->resume_path)
                                        <a href="{{ Storage::url($jobsapply->resume_path) }}" target="_blank"
                            ...
                            <div class="bg-white dark:bg-purple-800/30 p-3 rounded-lg">
                                <div class="flex items-center justify-between">
                                    ...
                                    @if($jobsapply?->cover_letter_path)
                                        <a href="{{ Storage::url($jobsapply->cover_letter_path) }}" target="_blank"
                                           class="px-3 py-1.5 text-xs font-medium text-purple-600 bg-purple-100 hover:bg-purple-200 dark:bg-purple-800 dark:text-purple-300 dark:hover:bg-purple-700 rounded-lg transition-colors">
                                            📝 View File
                                        </a>
                                    @else
                    ...
                    <div class="bg-purple-100 dark:bg-purple-900/40 p-3 rounded-lg">
                        ...
                        <div class="text-xs text-purple-600 dark:text-purple-400 space-y-1">
                            <p><span class="font-medium">Application ID:</span> #{{ $jobsapply?->id }}</p>
                            <p><span class="font-medium">Job ID:</span> #{{ $jobsapply?->job_id }}</p>
                            <p><span class="font-medium">User ID:</span> #{{ $jobsapply?->user_id }}</p>
                        </div>
                    </div>
                </div>

                    <!-- Career Objective Section -->
                    @if($jobsapply?->why_interested)
                    <section>
                        ...
                        <div class="bg-purple-50 dark:bg-purple-900/30 p-4 rounded-lg">
                            <div class="text-gray-700 dark:text-gray-300 leading-relaxed">
                                <p class="whitespace-pre-line">{{ $jobsapply?->why_interested }}</p>
                            </div>
                        </div>
                    </section>
                    @endif
                    ...
                    <section>
                        ...
                        <div class="bg-purple-50 dark:bg-purple-900/30 p-4 rounded-lg">
                            <div class="text-gray-700 dark:text-gray-300 leading-relaxed">
                                <p class="whitespace-pre-line">{{ $jobsapply?->cover_letter }}</p>
                            </div>
                        </div>
                    </section>
                    @endif
                    ...
                    @if($jobsapply?->linkedin_url || $jobsapply?->portfolio_url)
                    <section>
                        <h2 class="text-lg font-bold text-purple-800 dark:text-purple-300 mb-3 pb-2 border-b-2 border-purple-300 dark:border-purple-600">
                            🌐 PROFESSIONAL LINKS
                        </h2>
                        <div class="flex flex-wrap gap-3">
                            @if($jobsapply?->linkedin_url)
                            <a href="{{ $jobsapply->linkedin_url }}" target="_blank"
                            ...
                            @endif
                            @if($jobsapply?->portfolio_url)
                            <a href="{{ $jobsapply->portfolio_url }}" target="_blank"
                               class="px-4 py-2 text-sm font-medium text-purple-600 bg-purple-100 hover:bg-purple-200 dark:bg-purple-800 dark:text-purple-300 dark:hover:bg-purple-700 rounded-lg transition-colors">
                                🎨 Portfolio
                            </a>
                            @endif
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                                    <div class="bg-white dark:bg-purple-800/40 p-3 rounded border border-purple-200 dark:border-purple-600">
                                        ...
                                        <p class="font-semibold text-purple-900 dark:text-purple-100">{{ $jobsapply?->full_name }}</p>
                                    </div>
                                    <div class="bg-white dark:bg-purple-800/40 p-3 rounded border border-purple-200 dark:border-purple-600">
                                        <span class="text-purple-600 dark:text-purple-400 font-medium">Email:</span>
                                        <p class="font-semibold text-purple-900 dark:text-purple-100">{{ $jobsapply?->email }}</p>
                                    </div>
                                    <div class="bg-white dark:bg-purple-800/40 p-3 rounded border border-purple-200 dark:border-purple-600">
                                        <span class="text-purple-600 dark:text-purple-400 font-medium">Phone:</span>
                                        <p class="font-semibold text-purple-900 dark:text-purple-100">{{ $jobsapply?->phone_number }}</p>
                                    </div>
                                    <div class="bg-white dark:bg-purple-800/40 p-3 rounded border border-purple-200 dark:border-purple-600">
                                        <span class="text-purple-600 dark:text-purple-400 font-medium">LinkedIn:</span>
                                        <p class="font-semibold text-purple-900 dark:text-purple-100 text-xs break-all">{{ $jobsapply?->linkedin_url ?: '❌ Not provided' }}</p>
                                    </div>
                                    <div class="bg-white dark:bg-purple-800/40 p-3 rounded border border-purple-200 dark:border-purple-600">
                                        <span class="text-purple-600 dark:text-purple-400 font-medium">Portfolio:</span>
                                        <p class="font-semibold text-purple-900 dark:text-purple-100 text-xs break-all">{{ $jobsapply?->portfolio_url ?: '❌ Not provided' }}</p>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                                    <div class="bg-white dark:bg-purple-800/40 p-3 rounded border border-purple-200 dark:border-purple-600">
                                        ...
                                        <p class="font-semibold text-purple-900 dark:text-purple-100">
                                            {{ $jobsapply?->cover_letter ? strlen($jobsapply->cover_letter) . ' characters' : '❌ No cover letter text' }}
                                        </p>
                                    </div>
                                    <div class="bg-white dark:bg-purple-800/40 p-3 rounded border border-purple-200 dark:border-purple-600">
                                        <span class="text-purple-600 dark:text-purple-400 font-medium">Why Interested Length:</span>
                                        <p class="font-semibold text-purple-900 dark:text-purple-100">
                                            {{ $jobsapply?->why_interested ? strlen($jobsapply->why_interested) . ' characters' : '❌ No why interested text' }}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                                    <div class="bg-white dark:bg-purple-800/40 p-3 rounded border border-purple-200 dark:border-purple-600">
                                        ...
                                        <p class="font-semibold text-purple-900 dark:text-purple-100">{{ $jobsapply?->created_at?->format('Y-m-d H:i:s') }} ({{ $jobsapply?->created_at?->diffForHumans() }})</p>
                                    </div>
                                    <div class="bg-white dark:bg-purple-800/40 p-3 rounded border border-purple-200 dark:border-purple-600">
                                        <span class="text-purple-600 dark:text-purple-400 font-medium">Updated At:</span>
                                        <p class="font-semibold text-purple-900 dark:text-purple-100">{{ $jobsapply?->updated_at?->format('Y-m-d H:i:s') }} ({{ $jobsapply?->updated_at?->diffForHumans() }})</p>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="flex items-center justify-between p-6 border-t border-purple-200 dark:border-purple-700 bg-purple-50 dark:bg-purple-900/20">
                ...
                <div class="text-sm text-purple-600 dark:text-purple-400">
                    Application ID: #{{ $jobsapply?->id ?? 'N/A' }}
                </div>
                <button wire:click="closeModal"
